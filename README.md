...
<!-- Replace the line below with a GIF or screenshot of your deployed site -->
...
- **Power System** — Toggle an "inverted" mode that shifts the entire color palette, simulating a narrative event
...
- **SSE Real-Time Stats** — Live visitor count and uptime, zero external dependencies (no Socket.IO)
...
All personal content is stored in the `data/` directory — edit these files to make the site yours:

| File | What to Change |
|------|----------------|
| `data/projects.ts` | Your projects and portfolio items |
| `data/life.ts` | Games, travel, hobbies — your life outside code |
...
The template uses placeholder images from [placehold.co](https://placehold.co). Replace image URLs in your data files with your own hosted images. If using a CDN with image processing (like Cloudinary or Tencent COS), customize the `getProcessedImageUrl` function in `components/shared/LazyImage.tsx`.
...
├── lib/                # Blog utilities, image helpers
...
The visual language deliberately blurs the line between functional UI and narrative element. The green highlight color (`--ark-highlight-green`) serves double duty as both the interface accent and a reference to the fictional "Green Tide" consuming the world.

You are free to keep this narrative layer, adapt it to your own story, or strip it entirely and use the template as a pure visual framework.
